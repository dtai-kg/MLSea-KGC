...
def integrate_openml_tasks(tasks_df, targetpath, files, file_part, file_subpart):
    ...
    graph = morph_kgc.materialize(
        './morph_config/task_conf.ini', data_dict)
    print("RDF generated!")
    print("Saving to disk...")
    # for s,p,o in graph.triples((None, None, None)):
    #     print(s,p,o)

    filename = "openml_tasks_" + \
        str(file_part) + "_part_" + str(file_subpart) + ".nt"
...
def find_instance_count(db, graph):
    ...
    try:
        count = int(db.get_triples(query)[
                    "results"]["bindings"][0]["instanceCount"]["value"])
    except:
...
def integrate_openml_tasks_from_csv(datapath, targetpath, batch_offset, batch_size, update=False):
    ...
    while tasks_clearance == True:
        ...
        files, n_triples = integrate_openml_tasks(
            tasks_df, targetpath, files, file_part, file_subpart)
...
def integrate_openml(update=False):
    ...
    integrate_openml_tasks_from_csv(
        datapath, targetpath, tasks_offset, batch_size, update)
    integrate_openml_flows_from_csv(
        datapath, targetpath, flows_offset, batch_size, update)
    integrate_openml_datasets_from_csv(
        datapath, targetpath, datasets_offset, dataset_batch_size, update)
    integrate_openml_runs_from_csv(
        datapath, targetpath, runs_offset, batch_size, update)
